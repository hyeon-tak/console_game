...
{
    //현재 진행중의 버그
    //아이템의 장착 가능 갯수 제한이 안됨
    //장착중인 아이템의 이름이 제대로 출력이 안됨

    internal partial class Games
    {

        static Stack<Tuple<int, int>> PlayerPosition = new Stack<Tuple<int, int>>();
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine("1을 입력하면 맵으로 이동합니다. 다른 키를 누를 시 종료합니다.");
        ...
        static Player ChoiceJob()
        {
            ...
            static void WorldMap(Player player, Item item, EquipItem equipItem)
            {
                ...
                Console.WriteLine("I를 누르면 현재 상태와 인벤토리를 확인 할 수 있습니다.\nEnter를 누르면 상호작용을 합니다");
                ...
                switch (keyInfo.Key)                            //위에서 키 입력 받은거로 판단
                    {
                        case ConsoleKey.UpArrow:                    //입력받은 키가 위쪽 방향키면
                            if (field[playerY - 1, playerX] == ". ") //현재 좌표에서 Y좌표의 -1한 위치가 #이 아니면  
                            {
